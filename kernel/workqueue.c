...
struct worker_pool {
	...
	struct work_struct      idle_cull_work; /*删除idle worker的异步work
...
struct wq_flusher {
	...
	如果wq->nr_pwqs_to_flush变为0, 这里会被调用complete()
	flush completion */
};
...
/*
 * I: kthread_worker 用于释放 pwq。释放 pwq 时需要在持有 pool 锁的情况下切换到进程上下文,
 * 因此需要将释放操作转交给专用的 kthread_worker，以避免 A-A 死锁.
 这个worker运行pwq的release work
 */
...
/**
返回work关联的worker pool
通过pwq或者存储的poolid获取pool
...
static struct worker_pool *get_work_pool(struct work_struct *work)
{
	...
	/* work位于pwq上了已经, 这里直接返回pwq的pool */
	if (data & WORK_STRUCT_PWQ)
		return work_struct_pwq(data)->pool;
	/* 如果没有关联的pwq， 那么可能存储的有pool id */
...
/*
看看是不是,pool->worklist不空,并且没有正在运行的worker
			      ...
			      struct work_struct **nextp)
{
	 ...
	 * use NULL for list head.
	 把work->entry同链表的所有WORK_STRUCT_LINKED的work移动到head
	 */
	...
	*/
	...
	if (!pool->attrs->affn_strict &&
	    !cpumask_test_cpu(p->wake_cpu, pool->attrs->__pod_cpumask)) {
			/* 如果pool的affn不严格, 并且p的wake_cpu不在pool的cpu范围 */
		struct work_struct *work = list_first_entry(&pool->worklist,
						struct work_struct, entry);
		/* 从pool->attrs->__pod_cpumask掩码指定的cpu范围
		选择下一个cpu作为wake_cpu */
...
/**
看来是移动到对应pool的worklist上就活跃了
 * @description: 激活一个处于非活动状态的工作
 work是刚从pwq的inactive_works链表上取下来的
 * @return {*}
 */
static void pwq_activate_inactive_work(struct work_struct *work)
{
	...
	if (list_empty(&pwq->pool->worklist))
		pwq->pool->watchdog_ts = jiffies;/* 记录pool的worklist为空的最新时间 */
	/* 把work所在链表上面的所有WORK_STRUCT_LINKED的work移动到pool的worklist */
...
/* 
把pwq->inactive_works上的全部WORK_STRUCT_LINKED的work移动到pwq->pool->worklist上
*/
static void pwq_activate_first_inactive(struct pool_workqueue *pwq)
{
	/* 取出pwq的inactive_works链表上的第一个work */
 ...
 *
 在一个work被完成或者从pending队列中移除后，调用这个函数
 减少这个值, 这个值是flush的时候用于计数的
 ...
 * This function is safe to call from any context including IRQ handler.
 返回0表示本来没有pending
 返回1表示是pending状态,然后移除了
 返回-ENOENT表示正在被取消
 返回-EAGAIN表示现在处于pending状态,但是没有pool什么的
 */
			       ...
			       unsigned long *flags)
{
	...
	if (!test_and_set_bit(WORK_STRUCT_PENDING_BIT, work_data_bits(work)))
		return 0;/* 如果本来没有pending， 就返回 */

/* 现在说明这个work处于pending状态 */
	rcu_read_lock();
	/*
	...
	if (pwq && pwq->pool == pool) {
		...
		if (*work_data_bits(work) & WORK_STRUCT_INACTIVE)
			pwq_activate_inactive_work(work); /* 这里移动到pool的worklist */
			...
			struct list_head *head, unsigned int extra_flags)
{
	...
	/* 加入pool的worklist或者pwq的inactive_list */
...
/*
从wq_unbound_cpumask选择一个cpu来执行work
...
retry:
	...
	if (last_pool && last_pool != pool) {
		...
		/* 找到pool中正在执行work的worker */
		worker = find_worker_executing_work(last_pool, work);

		/* 这里是说如果worker就属于这个wq,那么就使用这个worker的pwq的pool?
		即使可能不是这个cpu的pwq和pool? */
...
/* 
culllist是pool的要清理的超时idle worker
*/
...
static void set_worker_dying(struct worker *worker, struct list_head *list)
{
	...
	/* 加入cull list和pool的要清理的list */
	list_move(&worker->entry, list);
	list_move(&worker->node, &pool->dying_workers);
}

/**
检查是否可以删除一些idle worker
运行清理的异步work
...
static void idle_worker_timeout(struct timer_list *t)
{
	...
	if (do_cull) /* 运行删除worker的异步work */
		queue_work(system_unbound_wq, &pool->idle_cull_work);
}

/**
pool的idle worker cull函数
用来清理一些worker
...
static void idle_cull_fn(struct work_struct *work)
{
	...
	while (too_many_workers(pool)) {
		...
		/* 取下一个worker, 计算清理的超时时间 */
		worker = list_entry(pool->idle_list.prev, struct worker, entry);
		expires = worker->last_active + IDLE_WORKER_TIMEOUT;

		if (time_before(jiffies, expires)) {/* 还没超时, 不用清理这个pool了 */
			mod_timer(&pool->idle_timer, expires);
			break;
		}
...
/**
wq->rescuer->task的线程函数
遍历mayday上面的pwq
把pool里的work取下来交给rescuer执行
====================
	...
	for_each_pwq(pwq, wq) {
		...
		if (flush_color >= 0) {
			...
			if (pwq->nr_in_flight[flush_color]) {
				/* 如果pwq->nr_in_flight[flush_color]>0
				pwq需要flush */
...
把barrier插入到work后面
 * @description: 
 * @param {wq_barrier} *barr
 * @param {bool} from_cancel
 * @return {*}
 */
...
/**
取消work,等待完成
 * @description: 取消一个work
 * @param {bool} is_dwork, 是不是delayed_work
 * @return {*}
 */
static bool __cancel_work_timer(struct work_struct *work, bool is_dwork)
{
	 ...
	 * visible there.
	 这里唤醒等待结构体, 唤醒里面的进程,也就是current进程
	 */
	smp_mb();
...
{
	local_irq_disable();
	/* 这里timer可能没有pending, 移除就移除了,
	有可能已经pending了, 也移除了, 返回1 */
...
static int init_worker_pool(struct worker_pool *pool)
{
	...
	/* timer到期了就清理太久idle的worker */
	timer_setup(&pool->idle_timer, idle_worker_timeout, TIMER_DEFERRABLE);
	/* 初始化清理idle worker的异步work */
...
static struct worker_pool *get_unbound_pool(const struct workqueue_attrs *attrs)
{
	...
	/* 没有在unbound_pool_hash哈希表上面找到指定的pool */
...
/*
 * 当一个未绑定的 pwq（pool_workqueue）引用计数为零且需要被销毁时,
 * 由 put_pwq() 调度到 pwq_release_worker 上执行.
 */
...
static void pwq_adjust_max_active(struct pool_workqueue *pwq)
{
	 ...
	 */
	if (!freezable || !workqueue_freezing) {/* 如果是一个非冻结可用的状态 */
		pwq->max_active = wq->saved_max_active;

		while (!list_empty(&pwq->inactive_works) &&
		       pwq->nr_active < pwq->max_active)
			pwq_activate_first_inactive(pwq); // 把pwq的inactive工作项激活,就是把它们移动到pool的worklist上

		kick_pool(pwq->pool);
	} else {
		/* 如果freezable并且workqueue_freezing */
		pwq->max_active = 0;
	}
...
/**
初始化pwq, 关联wq与pool
 * @description: 建立wq的pwq与worker pool的关联
 * @param {pool_workqueue} *pwq, 是wq的某cpu的pwq
 * @param {worker_pool} *pool. 此cpu的worker pool
 * @return {*}
 */
...
把pwq加入wq
 * @description: 
 * @param {pool_workqueue} *pwq, 是新建的pwq
 * @return {*}
 */
		      ...
		      const cpumask_var_t unbound_cpumask)
{
	...
	/* 把unbound_cpumask合并到attr */
	wqattrs_actualize_cpumask(new_attrs, unbound_cpumask);
	cpumask_copy(new_attrs->__pod_cpumask, new_attrs->cpumask);
	/* 创建一个新的pwq复制给ctx->dfl_pwq */
	ctx->dfl_pwq = alloc_unbound_pwq(wq, new_attrs);
	if (!ctx->dfl_pwq)
		goto out_free;

	/* 初始化ctx->pwq_tbl[cpu]每个成员 */
	for_each_possible_cpu(cpu) {
			  ...
			  int hotplug_cpu, bool online)
{
	...
	/* wq_unbound_cpumask添加到target_attr的cpu掩码 */
	wqattrs_actualize_cpumask(target_attrs, wq_unbound_cpumask);

	/* nothing to do if the target cpumask matches the current pwq
	更新计算target attr的pod mask */
...
static int alloc_and_link_pwqs(struct workqueue_struct *wq)
{
	...
	if (!wq->cpu_pwq)
		goto enomem;
/* 
看来是绑定cpu的wq有pwq, 这个好像是依赖pcp的全局pool worker的*/
	if (!(wq->flags & WQ_UNBOUND)) {
		/* 如果wq是绑定到cpu的 */
		for_each_possible_cpu(cpu) {
			/* 取出每个cpu的pwq */
...
static int init_rescuer(struct workqueue_struct *wq)
{
	...
	/* 创建rescuer的worker tsk
	用于处理mayday上的pwq的pool的work */
 ...
 * @description: 
 * @param {char} *fmt
 * @param {unsigned int} flags, wq的属性
...
static void unbind_workers(int cpu)
{
	...
	for_each_cpu_worker_pool(pool, cpu) {
		 ...
		 * is on the same cpu.
		 把pool的每一个worker设置为WORKER_UNBOUND
		 */
...
/**
设置pool的workers允许的cpu
允许系统全部cpu?
====================
一种情况是pool现在的cpu小于0, @cpu刚刚上线
...
/* 
应用新的wq_unbound_cpumask (@unbound_cpumask不同于现在的wq_unbound_cpumask就执行
这个函数)
========================
遍历每一个wq, 修改wq, 通过创建ctx来执行
执行的像是一个初始化wq的操作 */
static int workqueue_apply_unbound_cpumask(const cpumask_var_t unbound_cpumask)
{
	...
	list_for_each_entry(wq, &workqueues, list) {
		...
		/* 创建ctx, 携带了很多信息
		最主要的好像是这个发生变化了的unbound_cpumask掩码 */
...
/* 刚刚为一些wq创建了ctx, 放在了ctxs链表 */

/* 处理每一个预分配的ctx, 根据上一步的ret决定是commit还是abort */
	list_for_each_entry_safe(ctx, n, &ctxs, list) {
		if (!ret)
			apply_wqattrs_commit(ctx);/* 安装ctx */
		apply_wqattrs_cleanup(ctx);/* 这里清理每一个预分配的 */
	}
 ...
 * @description: 
 * @param {device} *dev
 * @param {device_attribute} *attr
 * @param {char} *buf, 里面是新的cpumask
 * @param {size_t} count
 * @return {*}
 */
