...
def unit_test():
    ...
    failures += unit_test_check_error("FULL GRAPH", unit_test_full_dump_output, buffer)
    #
    # Full caller dump
    #
    print_dbg("")
    ...
    failures += unit_test_check_error(
        "FULL CALLER", unit_test_full_caller_output, buffer
    )
    #
    ...
    failures += unit_test_check_error(
        "CALLER, NO_EXTERNS", unit_test_noexterns_caller_output, buffer
    )
    #
    ...
    failures += unit_test_check_error(
        "CALLER, MAX DEPTH 2", unit_test_maxdepth2_caller_output, buffer
    )
    ...
    failures += unit_test_check_error(
        "CALLER, MAX DEPTH 3", unit_test_maxdepth3_caller_output, buffer
    )
    #
    ...
    failures += unit_test_check_error(
        "CALLER, REGEX", unit_test_regex_caller_output, buffer
    )
    #
    # Full callee
    #
    ...
    failures += unit_test_check_error(
        "CALLEE, FULL", unit_test_full_callee_output, buffer
    )
    #
    # Max depth callee
    #
    ...
    failures += unit_test_check_error(
        "CALLEE, MAX DEPTH 4", unit_test_maxdepth4_callee_output, buffer
    )
    print_dbg("")
    ...
    failures += unit_test_check_error(
        "CALLEE, MAX DEPTH 5", unit_test_maxdepth5_callee_output, buffer
    )
    #
    # Show results
    #
...
def build_callee_info(function_db):
    for call, value in function_db.items():
        for callee in value["calls"]:
            if (
                callee in function_db
                and call not in function_db[callee]["callee_calls"]
            ):
...
def dump_path_ascii(path, reverse, **kwargs):
    ...
    if truncated or externs:
        ascii_path += ';\n"{}"{}{}'.format(
            function if not reverse else path[-1],
...
def dump_path(path, functions, function_name, **kwargs):
    ...
    #
    if (exclude is not None and re.match(exclude, function_name) is not None) or (
        max_depth > 0 and len(path) >= max_depth
    ):
        dump_path_ascii(path, reverse_path, stdio_buffer=std_buf, truncated=True)
        return
    ...
    if function_name in seen_in_path:
        if max_depth <= 0 or (len(path) + 1) <= max_depth:
            dump_path_ascii(path + [function_name], reverse_path, stdio_buffer=std_buf)
        return
    ...
    for caller in functions[function_name][call_index]:
        ...
        if caller in functions:
            ...
            if function_name != caller:
                dump_path(path + [function_name], functions, caller, **kwargs)
            else:
                #
                # This is a recurrence for this function, add it once
                #
                dump_path_ascii(
                    path + [function_name, caller], reverse_path, stdio_buffer=std_buf
                )
        ...
        #
            ...
            (exclude is None or re.match(exclude, caller) is None)
            and (max_depth <= 0 or (len(path) + 2) <= max_depth)
            and not no_externs
        ):
            children += 1
            dump_path_ascii(
                path + [function_name, caller],
                reverse_path,
                externs=True,
                stdio_buffer=std_buf,
            )
        else:
            print_buf(std_buf, '"{}" [color=red];'.format(function_name))
...
def dump_function_info(functions, function, details):
    ...
    print("  {}() {}".format(function, finfo["files"] if details else ""))
    if details:
...
def main():
    ...
    parser.add_argument(
        "--no-externs", help="Do not show external functions", action="store_true"
    )
    parser.add_argument(
        "--no-warnings", help="Do not show warnings on the console", action="store_true"
    )
    parser.add_argument(
        "--max-depth",
        metavar="DEPTH",
    ...
    parser.add_argument("--unit-test", help=argparse.SUPPRESS, action="store_true")
    ...
    if config.caller and config.callee:
        print_err(
            "ERROR: Either --caller or --callee option should be given, " "not both!"
        )
        return 1
    ...
    if config.exclude is not None:
        try:
            ...
            print_err(
                "ERROR: Invalid --exclude regular expression, "
    ...
    if not config.caller and not config.callee and config.max_depth:
        print_err(
            "ERROR: The --max_depth option is only valid with " "--caller or --callee!"
        )
        return 1
    ...
    call = re.compile(r"^.*\(call.*\"(?P<target>.*)\".*$")
        ...
        if match is not None:
            ...
            if function_name in functions:
                if not config.no_warnings:
                    print_err(
                        "WARNING: Function {} defined in multiple"
                        'files "{}"!'.format(
                            function_name,
                            ", ".join(
                                map(
    ...
    if config.debug:
        print_dbg(
            "[PERF] Processing {} RTL files took {:.9f} seconds".format(
    ...
    if config.debug:
        print_dbg(
            "[PERF] Building callee info took {:.9f} seconds".format(
    ...
    if config.functions != "&None":
        ...
        if config.functions == "&all":
            ...
            if config.functions in functions:
                ...
                print_err(
                    'ERROR: Can\'t find callee, "{}" in RTL data!'.format(config.callee)
                )
                return 1
        return 0
    ...
    if not config.caller and not config.callee:
        full_call_graph(functions, exclude=config.exclude, no_externs=config.no_externs)
    ...
    if config.callee and len(config.callee) != 0:
        for callee in config.callee:
            if callee not in functions:
                print_err('ERROR: Can\'t find callee "{}" in RTL data!'.format(callee))
                return 1
    ...
    #
        ...
        for caller in config.caller:
            if caller not in functions:
                print_err('ERROR: Can\'t find caller "{}" in RTL data!'.format(caller))
                return 1
        print("strict digraph callgraph {")
        for caller in config.caller:
    ...
    if config.debug:
        print_dbg(
            "[PERF] Generating .dot file took {:.9f} seconds".format(
                time.time() - start_time
            )
        )
